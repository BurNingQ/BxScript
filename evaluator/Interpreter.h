...
private:
    // ============================================================
    // 1. Statement 执行层 (Execute): 负责逻辑控制、变量声明、代码块
    // ============================================================
    ...
    // ============================================================
    // 2. Expression 求值层 (Evaluate): 负责数据计算、赋值、成员访问
    // ============================================================
    ...
    static void LoadModule(const ImportStatement *stmt, std::shared_ptr<Environment> env) {
        ...
        const auto moduleEnv = std::make_shared<Environment>();
